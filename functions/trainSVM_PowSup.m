...
L_comb      = NaN(size(cfg.train,3),size(gridC,2)); %NaN(size(cfg.train,3),size(ThU,2),size(ThL,2),size(gridC,2));
sens_comb   = NaN(size(cfg.train,3),size(gridC,2)); %NaN(size(cfg.train,3),size(ThU,2),size(ThL,2),size(gridC,2));
spec_comb   = NaN(size(cfg.train,3),size(gridC,2)); %NaN(size(cfg.train,3),size(ThU,2),size(ThL,2),size(gridC,2));
prec_comb   = NaN(size(cfg.train,3),size(gridC,2)); %NaN(size(cfg.train,3),size(ThU,2),size(ThL,2),size(gridC,2));
F_comb      = NaN(size(cfg.train,3),size(gridC,2)); %NaN(size(cfg.train,3),size(ThU,2),size(ThL,2),size(gridC,2));
...
for subj = cfg.train
    ...
    L_all = NaN(size(gridC)); %NaN(length(ThU),length(ThL),length(gridC));
    sens_all = NaN(size(gridC)); %NaN(length(ThU),length(ThL),length(gridC));
    prec_all = NaN(size(gridC)); %NaN(length(ThU),length(ThL),length(gridC));
    spec_all = NaN(size(gridC)); %NaN(length(ThU),length(ThL),length(gridC));
    F_all = NaN(size(gridC)); %NaN(length(ThU),length(ThL),length(gridC));
    
%     for p = 1:length(ThU)
%         for q = 1:length(ThL)
            
            [Area_conc, tStart_conc, fStart_conc, tWidth_conc, fWidth_conc] = getfeaturesTrain(dataBase(subj));
            ...
            for m = 1:size(gridC,2)
                ...
                fprintf('---- subject = %s, C = %g ----\n',dataBase(subj).sub_label,C)
