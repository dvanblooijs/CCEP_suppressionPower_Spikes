...
% detects early responses after Single Pulse Electrical Stimulation
    ...
    for trial = 1:size(epoch_sorted_avg,2)
        ...
        for elec=1:size(epoch_sorted_avg,1) % for each electrode
            ...
            elseif ~ismember(elec,cc_stimsets(trial,1:2))
                ...
                [samppos, amplpos]  = peakfinder(signal_new(1,round(fs*epoch_prestim+extrasamps):round(fs*(epoch_prestim+0.1))),sel,[],1);
                [sampneg, amplneg] = peakfinder(signal_new(1,round(fs*epoch_prestim+extrasamps):round(fs*(epoch_prestim + 0.1))),sel,[],-1);
        ...
        if size(cc_stimsets,2)>2 % stim current is also notified
