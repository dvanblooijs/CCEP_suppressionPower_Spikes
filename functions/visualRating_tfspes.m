function dataBase = visualRating_tfspes(dataBase,subj,lookPic)
...
for stimp = 1:size(dataBase(subj).ERSP.allERSPboot,1)
    for chan = 1:size(dataBase(subj).ERSP.allERSPboot,2)
        ...
        if ~ismember(chan,dataBase(subj).ERSP.cc_stimsets(stimp,:)) && lookPic(stimp,chan)==1 % && detected(stimp,chan) == 1
            ...
            if isfield(dataBase(subj).ERSP, 'Area')
            ...
            s = input(sprintf('%2.1f %% %s --- stimpair = %s-%s chan = %s --- Is this an ERSP? [y/n]: ',perc,dataBase(subj).sub_label, dataBase(subj).ERSP.cc_stimchans{stimp,:},dataBase(subj).ERSP.ch{chan}),'s');
