...
%             end
%
%         case 'sub-CCEPERSPUMCU02' % not really convincing spikes...
...
for subj = 1:size(dataBase,2)
    ...
    if any(contains(fieldnames(dataBase(subj).tb_electrodes),'ied'))
        idx_ied = contains(dataBase(subj).tb_electrodes.ied,'yes');

        
    else
        
        idx_ied = false(size(dataBase(subj).tb_electrodes,1),1);
    end

    IEDch = find(idx_ied ==1);
    IEDchan = dataBase(subj).tb_electrodes.name(idx_ied);
