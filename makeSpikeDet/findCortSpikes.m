...
function [Mrange,M] = findCortSpikes(data_stimfree,ccep_header)
...
close(h)
disp('The Mahalanobis distance is calculated!')
...
Pharmat = gevfit(M(j,:));
Mthresh(j) = Pharmat(3) +2*Pharmat(2); %2SD


    % per kanaal, locatie van M's en de waardes van M's die binnen grenzen zitten
    Mrange(j).chan = chan;
%    Mrange(j).locs = find(M(chan,:)>Mmin & M(chan,:)<Mmax);
